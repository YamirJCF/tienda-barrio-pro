-- Descripción del cambio: Actualizar rpc_procesar_venta_v2 para incluir cost_price en el histórico de sale_items y habilitar cálculo real de Ganancia Bruta (Analytics Phase 1)
-- Autor: Arquitecto de Datos (Antigravity)
-- Ref: FRD_015_CORRECCION_COSTO_VENTAS.md
...
DECLARE
    ...
    v_product_cost DECIMAL; -- NUEVO CAMPO: Para almacenar el costo real del producto al momento de la venta
BEGIN
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        ...
        -- MODIFICADO: Extraer también cost_price de la tabla products
        SELECT price, cost_price INTO v_product_price, v_product_cost 
        ...
        -- MODIFICADO: Insertar unit_cost (cost_price) garantizando un 0 si es nulo
        INSERT INTO public.sale_items (sale_id, product_id, quantity, unit_price, subtotal, unit_cost)
        VALUES (v_sale_id, v_product_id, v_quantity, v_product_price, v_subtotal, COALESCE(v_product_cost, 0));
