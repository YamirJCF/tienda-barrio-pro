-- ==============================================================================
-- MIGRATION: ADD UNIT COST TO SALE ITEMS
-- Fixes missing column unit_cost required by financial analytics
-- ==============================================================================

-- 1. Create the column
ALTER TABLE public.sale_items ADD COLUMN IF NOT EXISTS unit_cost DECIMAL(12, 2) DEFAULT 0;

-- 2. Backfill historical data from products
UPDATE public.sale_items si
SET unit_cost = COALESCE(p.cost_price, 0)
FROM public.products p
WHERE si.product_id = p.id AND si.unit_cost = 0;

-- 3. Update the POS handler to record the unit_cost at the time of sale
...
DECLARE
    ...
    v_product_price DECIMAL; v_product_cost DECIMAL; v_product_name TEXT; v_subtotal DECIMAL;
...
BEGIN
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        ...
        SELECT price, cost_price, name INTO v_product_price, v_product_cost, v_product_name FROM public.products WHERE id = v_product_id AND store_id = p_store_id;
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        v_product_id := (v_item->>'product_id')::UUID;
        v_quantity := (v_item->>'quantity')::DECIMAL;
        SELECT price, cost_price INTO v_product_price, v_product_cost FROM public.products WHERE id = v_product_id;
        v_subtotal := v_quantity * v_product_price;
        INSERT INTO public.sale_items (sale_id, product_id, quantity, unit_price, subtotal, unit_cost) VALUES (v_sale_id, v_product_id, v_quantity, v_product_price, v_subtotal, COALESCE(v_product_cost, 0));
