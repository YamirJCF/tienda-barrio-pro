...
| AU-03 | 001 | Teclado numérico virtual disponible en login | ✅ | Implementado `PinKeypad` (WO-006) |
| AU-04 | 001 | Mensaje "Esperando aprobación" si no tiene pase | ✅ | Captura error GATEKEEPER_PENDING |
| AU-05 | 001 | Botón "Reenviar alerta" (max 3 veces) visible en espera | ✅ | Implementado `GatekeeperPending` (WO-008) |
| AU-06 | 002 | Registro de tienda pide Nombre, Slug, Email Admin | ✅ | Slug auto-generado (ok) |
| AU-07 | 002 | Registro valida contraseña fuerte visualmente | ✅ | Validado Regex 8 chars + num/alpha |
...
| PE-05 | 003 | Opción "Desactivar" muestra confirmación (modal) | ✅ | Implementado `showDeactivateModal` |
...
| PO-02 | 004 | Apertura de caja pide PIN de seguridad | ✅ | Implementado `PinSetup/Challenge` (WO-004) |
...
| PO-08 | 007 | Checkout permite pago mixto o efectivo simple | ✅ | Implementado `MixedPayment` (WO-001) |
...
| IN-02 | 006 | Campo "Costo" oculto si no es Admin | ✅ | Oculto con `v-if="authStore.isAdmin"` |
...
| ID | FRD | Punto de Verificación Atómico | Estado | Nota |
|----|-----|-------------------------------|--------|------|
| CL-01 | 009 | Lista clientes muestra Balance y Cupo | ✅ | Cupo visible en tarjeta |
...
| CL-06 | 009 | Botón "Eliminar" deshabilitado si tiene deuda | ✅ | Implementado `SafeDeletion` (WO-003) |

### 2.6 Reportes y Finanzas (FRD-007)
| ID | FRD | Punto de Verificación Atómico | Estado | Nota |
|----|-----|-------------------------------|--------|------|
| RE-01 | 007 | Vista de Gastos permite registrar salidas | ✅ | ExpensesView funciona |
| RE-02 | 007 | Reporte de Ventas Diarias visible | ✅ | Implementado `ReportsView` (WO-002) |
| RE-03 | 007 | Reporte de Ganancias (Margen) visible | ✅ | Incluido en `ReportsContent` (WO-002) |
...
| UX-03 | QA | Skeletons visibles durante carga de datos | ✅ | Implementado Inventory/Clients (WO-005) |
...
|----------|--------|------------------------------------------|-----------|--------|
| AU-03 | Auth | **Falta Keypad en Login**: El usuario debe usar teclado del sistema. Se requiere `PinKeypad` para UX POS. | 🟠 Media | ✅ Corregido (WO-006) |
| AU-05 | Auth | **Falta Sala de Espera**: Al recibir `GATEKEEPER_PENDING`, solo muestra un texto rojo. Debería mostrar pantalla de espera con botón de reintento. | 🔴 Alta | ✅ Corregido (WO-PHASE4-001) |
| AU-07 | Auth | **Password Débil**: Registro permite "123456”. FRD exige 8 caracteres + alfanumérico. | 🟠 Media | ✅ Corregido (WO-PHASE4-001) |
| PE-05 | Personal | **Falta Confirmación al Desactivar**: El toggle de estado es instantáneo. FRD exige modal de confirmación para evitar accidentes. | 🔵 Baja | ✅ Corregido (WO-PHASE4-002) |
| PO-02 | POS | **Falta PIN en Apertura Caja**: El cajero puede abrir caja solo con el monto. Se requiere confirmar PIN para auditoría. | 🟠 Media | ✅ Corregido (WO-004) |
| IN-02 | Inventario | **Costo Visible sin Permisos**: El formulario de producto muestra el campo Costo a todos los usuarios. Debe ocultarse para empleados sin rol Administrativo. | 🟠 Media | ✅ Corregido (WO-PHASE4-002) |
| CL-01 | Clientes | **Falta Cupo en Lista**: La tarjeta de cliente en lista solo muestra el Balance, pero no el Cupo de Crédito asignado. | 🔵 Baja | ✅ Corregido (WO-PHASE4-002) |
| CL-06 | Clientes | **Eliminación Insegura**: El sistema permite eliminar clientes con deuda pendiente sin validación previa. Riesgo de pérdida financiera. | 🔴 Alta | ✅ Corregido (WO-003) |
| RE-02 | Reportes | **Falta Módulo de Reportes**: No existe vista dedicada para consultar historial de ventas o ganancias fuera del cierre de caja. | 🔴 Alta | ✅ Corregido (WO-002) |
| UX-03 | UX | **Falta Feedback de Carga (Skeletons)**: Las listas no muestran estado de carga. | 🔵 Baja | ✅ Corregido (WO-005) |
