...
const convertStock = (value: number, fromUnit: 'kg' | 'lb' | 'g', toUnit: 'kg' | 'lb' | 'g'): number => {
...
const save = () => {
  ...
  const stockToSave = (props.productId && originalStock.value && !stockWasModified.value)
    ? originalStock.value
  ...
  if (id) {
    ...
    if (product) {
      ...
      formData.value = {
        ...
        measurementUnit: product.measurementUnit !== 'un' ? product.measurementUnit as 'kg' | 'lb' | 'g' : 'lb',
...
watch(() => formData.value.stock, (newVal, oldVal) => {
...
watch(() => formData.value.measurementUnit, (newUnit, oldUnit) => {
  ...
  logger.log(`[ProductForm] Stock convertido: ${currentValue} ${oldUnit} → ${formatQuantity(convertedValue)} ${newUnit}`);
});
</script>
...
<template>
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="modelValue" class="fixed inset-0 z-50 flex items-end justify-center bg-gray-800/80"
        @click.self="close">
        <!-- Modal Container -->
        <div
          ...
          <div
            ...
            <div class="flex items-center justify-between px-4 pb-3">
              <div class="flex items-center gap-2">
                ...
                <span v-if="productId"
                  class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold" :class="formData.saleMode === 'weight'
            ...
            <form class="grid grid-cols-12 gap-3" @submit.prevent="save">
              ...
              <div class="col-span-12">
                ...
                <input v-model="formData.name"
                  ...
                  placeholder="Ej. Coca Cola 3L Original" type="text" required />
              </div>
              ...
              <div class="col-span-8 relative">
                ...
                <div class="relative">
                  ...
                  <span
                    class="material-symbols-outlined absolute right-2.5 top-2 text-gray-400 text-[20px] pointer-events-none">search</span>
                </div>
              </div>
                ...
                <div class="bg-gray-100 dark:bg-gray-800 p-1 rounded-lg flex h-10 shadow-inner">
                  <button type="button"
                    ...
                    <span class="material-symbols-outlined text-[16px]"
                      :class="formData.saleMode === 'unit' ? 'fill-1' : ''">package_2</span>
                    Unidad
                  </button>
                  <button type="button"
                    ...
                    <span class="material-symbols-outlined text-[16px]"
                      :class="formData.saleMode === 'weight' ? 'fill-1' : ''">scale</span>
                    Valor/Peso
                  </button>
                </div>
              </div>
              ...
              <div class="col-span-6 pt-2">
                ...
                <div class="relative">
                  ...
                  <input v-model="formData.cost"
                    ...
                    placeholder="0" type="number" step="0.01" />
                </div>
              </div>
              <div class="col-span-6 pt-2">
                <label
              ...
              <div v-if="isPriceBelowCost" class="col-span-12 -mt-1">
                <div
                  ...
                  <span class="text-xs font-medium text-amber-700 dark:text-amber-300">El precio de venta es menor al
                    costo</span>
                </div>
              </div>
              ...
              <div class="col-span-6">
                <label
                  ...
                  <span v-if="formData.saleMode === 'weight'" class="text-primary dark:text-primary-400">
                    ({{ formData.measurementUnit }})
                  </span>
                  <span v-else class="text-gray-400">(un)</span>
                </label>
                <input v-model="formData.stock"
                  class="w-full h-10 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 text-sm text-slate-900 dark:text-white placeholder:text-gray-400 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all shadow-sm"
              ...
              <div class="col-span-6">
                ...
                <input v-model="formData.minStock"
                  class="w-full h-10 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 text-sm text-slate-900 dark:text-white placeholder:text-gray-400 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all shadow-sm"
