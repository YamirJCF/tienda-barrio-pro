...
const transactions = computed(() =>
  clientsStore.getClientTransactions(clientId.value)
);

const availableCredit = computed(() =>
  clientsStore.getAvailableCredit(clientId.value)
);
...
<template>
  <div class="min-h-screen bg-background-light dark:bg-background-dark pb-24">
    <!-- Hero Header Section -->
    <div v-if="client" class="relative bg-slate-800 rounded-b-[2rem] shadow-xl overflow-hidden z-10">
      ...
      <div class="relative flex flex-col p-6 pb-8">
        ...
        <div class="flex items-center justify-between mb-6">
          ...
          <div class="flex gap-4 relative">
            <button @click="toggleOptionsMenu" aria-label="Opciones"
        ...
        <div class="flex flex-col items-center text-center">
          ...
          <p class="text-slate-400 text-sm font-medium tracking-wide">C.C. {{ formatCedula(client.cedula) }}</p>
        </div>

        <!-- Gigantic Balance -->
        <div class="flex flex-col items-center mt-6 mb-8">
          <span class="text-emerald-400 text-sm font-bold uppercase tracking-widest mb-2">Saldo Pendiente</span>
        ...
        <div class="bg-slate-700/50 rounded-xl p-4 border border-white/5 backdrop-blur-sm">
          <div class="flex justify-between items-end mb-2">
            ...
            <p class="text-emerald-400 text-xs font-bold">Disponibles: {{ formatCurrency(availableCredit) }}</p>
          </div>
          <!-- Progress Track -->
          <div class="relative h-3 w-full bg-slate-900/50 rounded-full overflow-hidden">
            ...
            <div
              ...
              :style="{ width: `${Math.min(creditUsagePercent, 100)}%` }"></div>
          </div>
    ...
    <main class="relative z-0 px-4 pt-6 pb-32 flex flex-col gap-4">
      ...
      <div v-else class="flex flex-col gap-3">
        <article v-for="tx in transactions" :key="tx.id"
          ...
          <div class="shrink-0 flex items-center justify-center size-12 rounded-full" :class="tx.type === 'payment'
          ...
          <div class="flex-1 min-w-0">
            <p class="text-slate-900 dark:text-white text-base font-bold truncate">{{ tx.description }}</p>
            <p class="text-slate-500 dark:text-slate-400 text-xs font-medium mt-0.5">{{ formatDate(tx.date) }}</p>
          </div>
          <div class="shrink-0 text-right">
            <p class="text-lg font-bold" :class="tx.type === 'payment'
      ...
      <Transition name="modal">
        <div v-if="showPaymentModal"
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-scale-in">
            ...
            <div class="mb-4">
              <label class="text-sm font-medium text-slate-600 dark:text-slate-400 mb-2 block">Monto del abono</label>
              <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-600 font-bold text-xl">$</span>
                <input v-model="paymentAmount" type="number" inputmode="numeric"
            ...
            <div class="flex gap-3">
              ...
              <button @click="registerPayment" :disabled="!paymentAmount"
