...
const openCashRegister = () => {
    ...
    openingAmount.value = '';
    // Redirigir a Admin después de abrir caja
    router.push('/admin');
};

const closeCashRegister = () => {
    salesStore.closeStore();
    // Limpiar gastos del día al cerrar caja
    ...
    <div class="flex flex-col h-screen bg-background-light dark:bg-background-dark overflow-hidden">
        ...
        <template v-if="!isStoreOpen">
            ...
            <div
                ...
                <button @click="openCashRegister"
                    class="flex w-full cursor-pointer items-center justify-center rounded-xl h-14 px-6 bg-primary hover:bg-blue-700 text-white text-lg font-bold leading-normal tracking-[0.015em] shadow-lg shadow-primary/30 transition-all active:scale-[0.98]">
                    Abrir Caja
                </button>
            </div>
        </template>
        ...
        <template v-else>
            ...
            <div class="flex-1 overflow-y-auto px-4 py-6 flex flex-col gap-6">
                ...
                <div class="flex items-center gap-3 px-1">
                    <div class="h-10 w-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
                        <span class="material-symbols-outlined text-slate-600 dark:text-slate-400">inventory_2</span>
                    </div>
                ...
                <div
                    ...
                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Resumen del Sistema</h3>
                    ...
                    <div class="flex justify-between items-center text-sm">
                        ...
                        <span class="text-green-600 font-semibold font-mono">+ ${{ formatCurrency(salesStore.todayCash)
                        }}</span>
                    </div>
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-slate-600 dark:text-slate-300">Salidas / Gastos</span>
                        <span class="text-red-600 font-semibold font-mono">- ${{ formatCurrency(todayExpenses) }}</span>
                    </div>
                    <div class="my-2 h-px bg-slate-100 dark:bg-slate-700 border-dashed"></div>
                    <div class="flex justify-between items-end">
                        <span class="text-slate-900 dark:text-white font-bold text-base pb-1">Debe haber en Cajón</span>
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div class="relative">
                        ...
                        <div v-else-if="hasDifference"
                            class="absolute inset-y-0 right-0 pr-5 flex items-center pointer-events-none">
                            <span class="material-symbols-outlined text-red-600 text-[28px] animate-pulse">error</span>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="hasDifference && !isPositiveDifference"
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-red-700 dark:text-red-300 text-sm">Faltante: <span
                                class="font-mono font-bold text-base">-${{ formatCurrency(difference.abs()) }}</span>
                        </p>
                    </div>
                </div>

                <!-- Feedback Message: Surplus State -->
                <div v-if="hasDifference && isPositiveDifference"
            ...
            <div
                ...
                <button @click="closeCashRegister"
                    class="flex w-full cursor-pointer items-center justify-center rounded-xl h-14 px-6 bg-primary hover:bg-blue-700 text-white text-lg font-bold leading-normal tracking-[0.015em] shadow-lg shadow-primary/30 transition-all active:scale-[0.98]">
