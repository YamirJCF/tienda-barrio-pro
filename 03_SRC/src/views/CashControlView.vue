    ...
    <div class="flex flex-col h-screen bg-background-light dark:bg-background-dark overflow-hidden">
        ...
        <template v-if="!isStoreOpen">
            ...
            <template v-if="currentStep === 'amount'">
                <div class="flex-1 flex flex-col items-center justify-center px-6 gap-8 pb-20">
                    ...
                    <div class="flex flex-col items-center gap-6">
                        ...
                        <div class="text-center space-y-2">
                            <h1 class="text-slate-900 dark:text-white tracking-tight text-3xl font-black leading-tight">
                                Iniciar Jornada
                            </h1>
                            <p
                    ...
                    <div class="w-full max-w-xs space-y-3">
                        <label class="text-slate-700 dark:text-slate-300 text-sm font-semibold pl-1 block"
                            for="initial-base">
                        ...
                        <div class="relative group">
                            ...
                            <input v-model="openingAmount" ref="openingInput"
                                ...
                                id="initial-base" inputmode="decimal" placeholder="0" type="number" />
                        </div>

                        <!-- PIN Not Configured Warning -->
                        <div v-if="!hasPinConfigured"
                ...
                <div
                    ...
                    <button @click="goToPinStep" :disabled="!hasPinConfigured"
                ...
                <div class="flex-1 flex flex-col items-center justify-center px-6 gap-6">
                    <div class="text-center">
                        ...
                        <p class="text-2xl font-bold text-slate-900 dark:text-white">${{
                            formatCurrency(parseFloat(openingAmount) || 0) }}</p>
                    </div>
                    ...
                    <PinKeypad v-else :length="6" :error="pinError" :disabled="isLoading"
                        @complete="handlePinComplete" />

                    <button @click="currentStep = 'amount'"
        ...
        <template v-else>
            ...
            <header
                ...
                <h2 class="text-slate-900 dark:text-white text-lg font-bold leading-tight flex-1 text-center pr-12">
                    Arqueo de Caja</h2>
            </header>
            ...
            <template v-if="currentStep === 'amount'">
                ...
                <div class="flex-1 overflow-y-auto px-4 py-6 flex flex-col gap-6">
                    ...
                    <div class="flex items-center gap-3 px-1">
                        <div
                            ...
                            <span
                                class="material-symbols-outlined text-slate-600 dark:text-slate-400">inventory_2</span>
                        </div>
                    ...
                    <div
                        ...
                        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Resumen del Sistema
                        </h3>
                        ...
                        <div class="flex justify-between items-center text-sm">
                            ...
                            <span class="text-green-600 font-semibold font-mono">+ ${{
                                formatCurrency(salesStore.todayCash)
                                }}</span>
                        </div>
                        <div class="flex justify-between items-center text-sm">
                            <span class="text-slate-600 dark:text-slate-300">Salidas / Gastos</span>
                            <span class="text-red-600 font-semibold font-mono">- ${{ formatCurrency(todayExpenses)
                                }}</span>
                        </div>
                        <div class="my-2 h-px bg-slate-100 dark:bg-slate-700 border-dashed"></div>
                        <div class="flex justify-between items-end">
                            <span class="text-slate-900 dark:text-white font-bold text-base pb-1">Debe haber en
                                Cajón</span>
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="relative">
                            ...
                            <input v-model="closingAmount" ref="closingInput" :class="[
                            ...
                            ]" id="real-count" inputmode="decimal" placeholder="0" type="number" />
                            <!-- Status Icon inside input -->
                            <div v-if="closingAmount && difference.eq(0)"
                            ...
                            <div v-else-if="hasDifference"
                                class="absolute inset-y-0 right-0 pr-5 flex items-center pointer-events-none">
                                <span
                                    class="material-symbols-outlined text-red-600 text-[28px] animate-pulse">error</span>
                            </div>
                        </div>
                    </div>

                    <!-- Feedback Messages -->
                    <div v-if="hasDifference && !isPositiveDifference"
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-red-700 dark:text-red-300 text-sm">Faltante: <span
                                    class="font-mono font-bold text-base">-${{ formatCurrency(difference.abs())
                                    }}</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div v-if="hasDifference && isPositiveDifference"
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-blue-700 dark:text-blue-300 text-sm">Excedente: <span
                                    class="font-mono font-bold text-base">+${{ formatCurrency(difference) }}</span></p>
                        </div>
                    </div>
                ...
                <div
                    ...
                    <button @click="goToPinStep" :disabled="!hasPinConfigured || !closingAmount"
                        class="flex w-full cursor-pointer items-center justify-center rounded-xl h-14 px-6 bg-primary hover:bg-blue-700 disabled:bg-slate-400 disabled:cursor-not-allowed text-white text-lg font-bold leading-normal tracking-[0.015em] shadow-lg shadow-primary/30 transition-all active:scale-[0.98]">
            ...
            <template v-if="currentStep === 'pin'">
                <div class="flex-1 flex flex-col items-center justify-center px-6 gap-6">
                    <div class="text-center space-y-2">
                        ...
                        <p class="text-2xl font-bold text-slate-900 dark:text-white">${{
                            formatCurrency(parseFloat(closingAmount) || 0) }}</p>
                        <p v-if="hasDifference" :class="isPositiveDifference ? 'text-blue-600' : 'text-red-600'"
                    ...
                    <p class="text-slate-700 dark:text-slate-300 font-semibold">Ingresa tu PIN para confirmar:</p>
