...
const formattedCash = computed(() => {
  return `$ ${salesStore.currentCash.toDecimalPlaces(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}`;
});

const formattedTodayTotal = computed(() => {
  return `$ ${salesStore.todayTotal.toDecimalPlaces(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}`;
});

const formattedTodayFiado = computed(() => {
  return `$ ${salesStore.todayFiado.toDecimalPlaces(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}`;
});
...
<template>
  <div class="pb-24 bg-gray-50 dark:bg-background-dark min-h-screen">
    <!-- Header -->
    <header class="sticky top-0 z-30 bg-white dark:bg-surface-dark px-4 py-3 border-b border-gray-200 dark:border-gray-800 shadow-sm flex items-center justify-between">
      <h1 class="text-xl font-bold tracking-tight text-slate-900 dark:text-white font-display">Mi Tiendita</h1>
      <div class="flex items-center gap-4">
        <button class="relative text-slate-600 dark:text-slate-300">
          <span class="material-symbols-outlined text-[26px]">notifications</span>
          <span class="absolute top-0 right-0 h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white dark:ring-surface-dark"></span>
        </button>
        <div class="h-8 w-8 overflow-hidden rounded-full border border-gray-200 dark:border-gray-700 bg-gray-100">
    ...
    <main class="p-4 flex flex-col gap-6 max-w-md mx-auto">
      ...
      <!-- Info Banner -->
      <div v-if="!isStoreOpen && totalProducts === 0" class="relative overflow-hidden rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 p-4 shadow-sm">
      ...
      <div class="grid grid-cols-2 gap-3">
        <!-- Caja Real -->
        <div class="flex flex-col justify-between gap-3 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-surface-dark p-4 shadow-sm">
          <div class="rounded-lg bg-green-50 dark:bg-green-900/20 p-2 w-fit text-green-600 dark:text-green-400">
            <span class="material-symbols-outlined text-[28px]">payments</span>
          </div>
          <div>
            <p class="text-slate-500 dark:text-slate-400 text-[10px] font-semibold uppercase tracking-wide">Caja Real</p>
            <h3 class="text-xl font-bold text-slate-900 dark:text-white">{{ formattedCash }}</h3>
          </div>
        </div>

        <!-- Ventas Hoy -->
        <div class="flex flex-col justify-between gap-3 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-surface-dark p-4 shadow-sm">
          <div class="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-2 w-fit text-blue-600 dark:text-blue-400">
            <span class="material-symbols-outlined text-[28px]">show_chart</span>
          </div>
          <div>
            <p class="text-slate-500 dark:text-slate-400 text-[10px] font-semibold uppercase tracking-wide">Ventas Hoy</p>
            <h3 class="text-xl font-bold text-slate-900 dark:text-white">{{ formattedTodayTotal }}</h3>
            <p class="text-xs text-slate-400 mt-0.5">{{ salesStore.todayCount }} ventas</p>
          </div>
        </div>

        <!-- Por Cobrar -->
        <div class="flex flex-col justify-between gap-3 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-surface-dark p-4 shadow-sm">
          <div class="rounded-lg bg-orange-50 dark:bg-orange-900/20 p-2 w-fit text-orange-600 dark:text-orange-400">
            <span class="material-symbols-outlined text-[28px]">person</span>
          </div>
          <div>
            <p class="text-slate-500 dark:text-slate-400 text-[10px] font-semibold uppercase tracking-wide">Por Cobrar</p>
            <h3 class="text-xl font-bold text-slate-900 dark:text-white">{{ formattedTodayFiado }}</h3>
          </div>
        </div>

        <!-- Inventario -->
        <div class="flex flex-col justify-between gap-3 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-surface-dark p-4 shadow-sm">
          <div class="rounded-lg bg-purple-50 dark:bg-purple-900/20 p-2 w-fit text-purple-600 dark:text-purple-400">
            <span class="material-symbols-outlined text-[28px]">inventory_2</span>
          </div>
          <div>
            <p class="text-slate-500 dark:text-slate-400 text-[10px] font-semibold uppercase tracking-wide">Inventario</p>
            <h3 class="text-xl font-bold text-slate-900 dark:text-white">{{ totalProducts }} Prod.</h3>
            <p v-if="inventoryStore.lowStockProducts.length > 0" class="text-xs text-red-500 mt-0.5">
      ...
      <section>
        ...
        <div class="flex flex-col overflow-hidden rounded-xl bg-white dark:bg-surface-dark shadow-sm border border-slate-100 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800">
          <button 
          ...
          >
            <div class="flex items-center gap-3">
              <div class="flex h-10 w-10 items-center justify-center rounded-full bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400">
          ...
          >
            <div class="flex items-center gap-3">
              <div class="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300">
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="openingModalOpen" class="fixed inset-0 z-50 flex items-end justify-center">
          ...
          <div class="relative w-full max-w-md bg-white dark:bg-surface-dark rounded-t-3xl p-6 shadow-2xl animate-slide-up">
            <div class="mx-auto mb-6 h-1.5 w-12 rounded-full bg-slate-200 dark:bg-slate-700 cursor-pointer" @click="openingModalOpen = false"></div>
            <div class="flex flex-col gap-6">
              <div class="text-center">
                <div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 ring-1 ring-green-100 dark:ring-green-900">
